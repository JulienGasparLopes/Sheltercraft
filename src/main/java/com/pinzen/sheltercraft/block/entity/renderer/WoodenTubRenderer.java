...
public class WoodenTubRenderer implements BlockEntityRenderer<WoodenTubBE> {

    private final ItemRenderer itemRenderer;

    public WoodenTubRenderer (BlockEntityRendererProvider.Context context) {
        this.itemRenderer = context.getItemRenderer();
    }

    @Override
    public void render(WoodenTubBE woodenTubBE, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay, Vec3 cameraPos) {
        FluidStack fluidStack = woodenTubBE.getStoredFluid();

        ItemStack itemStack = woodenTubBE.getProcessingItem();
        ...
        if (!fluidStack.isEmpty()) {
            ...
            renderCuboid(
                    ...
                    0.95f, maxY, 0.95f,
                    packedLight,
                    woodenTubBE.isFluidTanned() // isTanned flag to determine if the tub is tanned
            );
            poseStack.popPose();
        }
        ...
        if (!itemStack.isEmpty()) {
            ...
            poseStack.translate(0.5, 0.1, 0.3);
            ...
            itemRenderer.renderStatic(itemStack, ItemDisplayContext.GROUND, packedLight, packedOverlay, poseStack, (layer) -> consumer, woodenTubBE.getLevel(), 0);
            poseStack.popPose();
        }
    }
